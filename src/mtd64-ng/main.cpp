...
int main() {
	...
	openlog("mtd64-ng", LOG_PID, 0); // Opening syslog connection
	...
	/* Starting server*/
	syslog(LOG_DAEMON | LOG_INFO, "Starting mtd64-ng...");
	struct sigaction sact;
	memset(&sact, 0x00, sizeof(sact));
	sact.sa_handler = shutdown;
	sact.sa_flags = 0;
	sigaction(SIGTERM, &sact, NULL); // Registering SIGTERM handler for clean shutdown
    server = new Server; // Creating new Server instance
    try {
		server->loadConfig("/etc/mtd64-ng.conf"); // Loading server config
	...
	delete server;
	syslog(LOG_DAEMON | LOG_INFO, "Stopping mtd64-ng..");
