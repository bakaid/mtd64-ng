...
class DNSClient: public DNSSource {
	private:
		Server& dns_server_; /**< The Server, used to access configuration settings. */
	...
	public:
		...
		/**
		 * Uses a configured DNS server (and the configured selection mode) to get an answer.
		 ...
		 */
		ssize_t sendQuery(uint8_t* query, size_t query_len, uint8_t* answer, size_t answer_len) override;
};
