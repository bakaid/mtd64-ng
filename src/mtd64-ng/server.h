...
class Server
{
	...
	private:
		...
		selectionMode sel_mode_;	/**< DNS server selection mode: (1) means round-robin, (2) means random */
		std::atomic<int> rr_;	/**< The sequence number of the DNS server which is actually in use in round-robin mode */

		struct in6_addr ipv6_;	/**< For checking if the address is valid, and used later for conversion, too */
		unsigned char ipv6_prefix_;	/**< Prefix length for IPv4 embedded IPv6 addresses */
		...
		short int response_maxlength_;	/**< Maximum legth of the IPv6 DNS response packet (UDP payload) */
		 ...
		 * @param v6 the buffer to store the synthesized IPv6 address (at least 16 bytes)
		 */
