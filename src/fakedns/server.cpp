...
bool Server::loadConfig(const char* filename) {
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if (strlen(begin) >= strlen("have-AAAA") && !strncmp(begin, "have-AAAA", strlen("have-AAAA"))) {
			...
			if (*begin == '1') {
				...
				if (sscanf(begin, "%lf", &aaaa_probability_) != 1 || (aaaa_probability_ < 0.0 || aaaa_probability_ > 1.0)) {
					aaaa_mode_ = NO;
					syslog(LOG_WARNING, "Invalid hava-AAAA at line %d. Defaulting to 0\n", linecount);
					continue;
				}
			}
		} else if (strlen(begin) >= strlen("debug") && !strncmp(begin, "debug", strlen("debug"))) {
			begin += strlen("debug");
			while (*begin != '\0' && isspace(*begin)) begin++;
...
void Server::start() {
	...
	if (bind(sock6fd_, reinterpret_cast<struct sockaddr*>(&fakednssrv_addr_), sizeof(fakednssrv_addr_)) == -1) {
		std::stringstream ss;
	...
	while (!pool_->isStopped()) {
		...
		if ((recvlen = recvfrom(sock6fd_, buffer, response_maxlength_, 0, reinterpret_cast<struct sockaddr*>(&sender), &sender_slen)) <= 0) {
			delete[] buffer;
			if (errno == EMSGSIZE) {
				syslog(LOG_DAEMON | LOG_WARNING, "The received message from IPv6 client is longer than %hd bytes. Ignored", response_maxlength_);
				continue;
			} else if (errno == EINTR) {
				break;
			} else {
				syslog(LOG_DAEMON | LOG_WARNING, "recvfrom() failure: %d (%s)", errno, strerror(errno));
				continue;
			}
		}
		inet_ntop(AF_INET6, &sender.sin6_addr, client_ip, INET6_ADDRSTRLEN);
		syslog(LOG_DAEMON | LOG_INFO, "Received packet from [%s]:%hu, length %zd", client_ip, ntohs(sender.sin6_port), recvlen);
