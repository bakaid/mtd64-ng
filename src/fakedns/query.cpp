...
void Query::operator()() {
	...
	if (header->qr() == 0 && header->opcode() == DNSHeader::OpCode::Query) {
		try {
			...
			if (sscanf(buffer, "%hhu-%hhu-%hhu-%hhu.dns64perf.test.", ip, ip+1, ip+2, ip+3) != 4) {
			...
			/* Copy question */
			memcpy(answer_data + answer_len, packet.question_[0].begin_, packet.question_[0].size());
			...
			if (packet.question_[0].qtype() == QType::A) {
				...
				*ptr = htons((packet.question_[0].begin_ - packet.begin_) | 0xc000);
				answer_len += 2;
				uint16_t* qtype = reinterpret_cast<uint16_t*>(answer_data + answer_len);
				*qtype = htons(QType::A);
				answer_len += 2;
				uint16_t* qclass = reinterpret_cast<uint16_t*>(answer_data + answer_len);
				*qclass = htons(QClass::IN);
				answer_len += 2;
				uint32_t* ttl = reinterpret_cast<uint32_t*>(answer_data + answer_len);
				*ttl = htonl(0);
				answer_len += 4;
				uint16_t* rdlength = reinterpret_cast<uint16_t*>(answer_data + answer_len);
				*rdlength = htons(4);
				answer_len += 2;
				...
				if (server_.aaaa_mode_ == Server::aaaaMode::YES ||
					(server_.aaaa_mode_ == Server::aaaaMode::PROBABILITY && ((double)rand() / RAND_MAX) <= server_.aaaa_probability_)) {
						header->ancount(1);
						/* Add answer label */
						uint16_t* ptr = reinterpret_cast<uint16_t*>(answer_data + answer_len);
						*ptr = htons((packet.question_[0].begin_ - packet.begin_) | 0xc000);
			...
			/* Send answer */
			if (sendto(server_.sock6fd_, answer_data, answer_len, 0, (struct sockaddr*) &sender_, sizeof(sender_)) == -1) {
